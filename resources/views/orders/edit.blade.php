...
@section('content')
    ...
    <form class="form-create"
          action="{{ route('orderEdit', [$gameAlias, $serviceAlias, $id]) }}"
          method="post"
                ...
                <div class="form-group">
                    <ul>
                        @foreach(json_decode($order->properties) as $file)
                            <input disabled type="text" class="form-control" placeholder="{{ $file->placeholder }}" value="{{ $file->name }}">
                        @endforeach
                    </ul>
                </div>
                <div class="row">
...
{{--                            <select disabled class="form-control" name="{{ $value['name'] }}">--}}
{{--                                <option selected disabled>{{ $value['placeholder'] }}</option>--}}
{{--                                @foreach($value['content'] as $item)--}}
{{--                                    @if($order->service->name === $item['name'])--}}
...
{{--                    @endforeach--}}
                    ...
                    <div class="form-group col-6">
                        <p>Цена:</p>
                        <input name="cost" type="text" class="form-control" placeholder="Стоимость" value="{{ $order->cost }}">
                    </div>
                ...
                <div class="form-group">
                    <p>Описание:</p>
                    <textarea name="description" class="form-control" placeholder="Описание" rows="3">{{ $order->description }}</textarea>
                </div>
            ...
            <div class="col-4">
                <div id="upload-container" class="form-group">
                    ...
                    <div>
                        ...
                        <label for="file-input">Загрузить дополнительные скриншоты</label>
                    </div>
                </div>
            </div>
        </div>
